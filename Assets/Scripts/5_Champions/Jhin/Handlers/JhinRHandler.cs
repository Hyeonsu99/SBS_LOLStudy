...
public class JhinRHandler : MonoBehaviour, IInputOverride
{
    ...
    public void StartUltimate(Vector3 targetPos, int level)
    {
        ...
        _playerController.SetInputOverride(this);
        ...
        _playerController.HandleCameraOverride(true);
    ...
    private void EndUltimate()
    {
        ...
        _playerController.SetInputOverride(null);
        // 카메라 복귀
        //_playerController.SetCameraOverride(false, Vector3.zero, 0);
        _playerController.HandleCameraOverride(false);
    ...
    public bool OnSkillInput(SkillCommand command)
    {
        if(command == SkillCommand.R)
        {
            Fire();
            return true;
        }
