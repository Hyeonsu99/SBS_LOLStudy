...
public class JhinRProjectile : MonoBehaviour
{
                           ...
                           float slowPer, float slowDur, bool isFourth)
    {
        ...
        _ownerSkillHandler = owner.GetComponent<SkillHandler>();
    ...
    private void ApplyEffects(UnitStat target)
    {
        ...
        // 진 표식이 적용가능한 상태(W를 1레벨 이상 배웠고 W가 사용 가능 상태 혹은 쿨타임이 4초 이하로 남았을 때...
        var wSlot = _ownerSkillHandler.GetSKillSlot(SkillCommand.W);
        ...
        {
            // 진 표식 적용
            target.ApplyEffect(EffectType.JhinMark, ModType.Flat, 4.0f, 0f);
        }
